...
get route with query parameters that returns all uploads that fit
...
//   uploads: UploadsResponseStructure[],
...
router.get("/uploads", (req: Request, res: Response) => {
  const maxCount =
    typeof req.query.maxCount === "string"
      ...
      : -1;
  const sortedBy =
    <Database.SortedBy>req.query.sortedBy ?? Database.SortedBy.Updoots;
  const direction =
    <Database.Direction>req.query.direction ?? Database.Direction.Descending;
  Database.getUploads(maxCount, sortedBy, direction)
    .then((response: Database.UploadsResponseStructure[] | string) => {
...
router.get("/uploadsByColumnValue", (req: Request, res: Response) => {
  if (
    ...
    const direction =
      <Database.Direction>req.query.direction ?? Database.Direction.Descending;
    Database.getUploadsByColumnValue(req.query.columnID, req.query.columnValue)
      .then((response: Database.UploadsResponseStructure[] | string) => {
        const successful: boolean =
          Boolean(response) &&
          !Object.prototype.hasOwnProperty.call(response, "message");
      ...
      if (typeof files !== "string") {
        ...
        for(const fileKey of fileKeys) {
          ...
          for(let i = 0; i < fileArray.length; i++) {
            ...
            await Database.insertUploads(fileLocalPath, userID);
